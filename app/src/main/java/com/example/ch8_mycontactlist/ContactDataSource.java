...
public class ContactDataSource {
    ...
    public boolean insertContact(Contact c) {
        ...
        try {
            ...
            if (c.getPicture() != null) {
                //to store bitmap to database, must first be converted to byte array
    ...
    public Contact getSpecificContact(int contactId) {
        ...
        if (cursor.moveToFirst()) {
            ...
            //doesn't need to be in getContacts() method bc picture won't be used by whole set of contacts
            //returning picture from db is essentially reverse process from saving to db
            byte[] photo = cursor.getBlob(10);
            if (photo != null) {
                ByteArrayInputStream imageStream = new ByteArrayInputStream(photo);
                //conversion from byte array back to bitmap will cause crash if no picture stored
